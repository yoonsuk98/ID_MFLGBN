...
def make_model(args, opts, pe):
    model = RAMiT(target_mode="light_dn",
                  img_norm="True",
...
class RAMiT(nn.Module):
    def __init__(self, in_chans=3, dim=64, depths=(6, 4, 4, 6), num_heads=(4, 4, 4, 4), head_dim=None,
                 ...
                 norm_layer=ReshapeLayerNorm, tail_mv=2, target_mode='light_dn',
                 img_norm=True,
                 attn_drop=0.0, proj_drop=0.0, drop_path=0.0, helper=True, **kwargs):
        ...
        self.stage1 = EncoderStage(depths[0], dim, num_heads[0], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, self.act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, self.my_act)
        self.down1 = Downsizing(dim, dim, norm_layer, mv_ver, self.my_act)
        self.stage2 = EncoderStage(depths[1], dim, num_heads[1], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, self.act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, self.my_act)
        self.down2 = Downsizing(dim, dim, norm_layer, mv_ver, self.my_act)
        self.stage3 = EncoderStage(depths[2], dim, num_heads[2], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, self.act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, self.my_act)
        self.bottleneck = Bottleneck(dim, len(depths) - 1, self.act_layer, norm_layer, mv_ver, self.my_act)
        self.stage4 = EncoderStage(depths[3], dim, num_heads[3], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, self.act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, self.my_act)
        self.attn_mix = HRAMi(dim, 3, 1, mv_ver, self.my_act)
        self.to_target = Reconstruction(target_mode, dim, 3, 1, tail_mv, mv_ver, self.my_act, exp_factor, expand_groups)
